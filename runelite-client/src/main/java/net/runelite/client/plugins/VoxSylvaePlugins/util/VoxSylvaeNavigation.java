...
public class VoxSylvaeNavigation extends Script {
    ...
    private void checkPlayerTeleports() {
        int magicLevel = Rs2Player.getSkillLevel(Rs2Player.SKILL.MAGIC);
        for (Map.Entry<String, TeleportInfo> entry : availableTeleports.entrySet()) {
            TeleportInfo info = entry.getValue();
            if (magicLevel >= info.requiredLevel && Rs2Inventory.hasAllItems(info.requiredItems)) {
    ...
    public boolean walkToArea(WorldPoint centerPoint, int radius) {
        return Rs2Walker.walkToArea(centerPoint, radius);
    }
    ...
    public static boolean prepareForTeleport(String teleportName) {
        TeleportInfo teleport = availableTeleports.get(teleportName);
        if (teleport == null) {
        ...
        if (!missingItems.isEmpty()) {
            return retrieveItemsFromBank(missingItems);
        }
    ...
    public boolean navigateToArea(String areaName) {
        ...
        if (currentLocation != null) {
            WorldPoint destination = currentLocation.getWorldPoint();
            
            // First, try to teleport close to the destination
            String nearestTeleport = findNearestTeleport(destination);
